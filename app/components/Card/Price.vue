  ...
  <div
    ...
    <ul class="flex w-full flex-col gap-3 md:gap-5">
      <li v-for="{ icon, title } in points" :key="icon" class="relative flex w-full items-center gap-3 overflow-hidden rounded bg-light-400 px-4 py-3 text-center dark:bg-dark-400">
        <NuxtIcon :name="`local:${icon}`" class="text-[24px] text-primary-500 md:text-[36px]" />
        <span class="text-left text-2xs md:text-base">{{ title }}</span>
      </li>
    </ul>
    <button
      class="rounded bg-primary-500 px-7 py-2 text-xs font-semi-bold text-white transition-colors duration-300 ease-out hover:bg-primary-400 md:px-10 md:py-4 md:text-lg"
      @click="emit('contact')">
      Contact
    </button>
  </div>
